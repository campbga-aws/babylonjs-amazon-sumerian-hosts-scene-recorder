...
async function loadDemo(createScene) {
  ...
  // Create Audio Node
  var outputNode = Engine.audioEngine.audioContext.createMediaStreamDestination();
  Engine.audioEngine.masterGain.connect(outputNode);

  // If the VideoRecorder is supported by the browser, start the recording with the Audio Node
  if (VideoRecorder.IsSupported(engine)) {
    var recorder = new VideoRecorder(engine, { audioTracks: outputNode.stream.getAudioTracks() });
    recorder.startRecording(hostConfig['recordingTitle'], hostConfig['recordingDuration']);
  }
